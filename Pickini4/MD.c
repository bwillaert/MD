     ...
     if (!calibration_delay )
     {
        ...
        {   
          ...
          else
          {
            ...
            // Except the first time where old_calibration_step is invalid
            if ((calibration_steps != 16) &&(old_calibration_step != calibration_step))
            {
              // sign reversal
              calibration_steps = 0;
            }   
  ...
  //    static unsigned long old_pulsevalue = 0;
     ...
     if (!measurecnt)
     {
        ...
        if (!startup_delay)
        {
           ...
           if (!PP_BUTTON)
           {
              pulse_average += (accumulate_measure_cnt<<4);
           }
...
void main()
{
   ...
   // timer 1 gating
    ...
    DACCON1 = DAC_value;         // 5V / 32 * DAC_value
